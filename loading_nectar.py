...
def get_valid_convs(d, condition=4, condition_type="ge"):
    ...
    for i in range(len(d)):
        chosen_conversation = d['chosen'][i]
        rejected_conversation = d['rejected'][i]
        if chosen_conversation[-1]['content'].lstrip().rstrip() == rejected_conversation[-1]['content'].lstrip().rstrip():
            continue
        # getting the conv
        dialog = []
        break_flag = 0
        for conv in chosen_conversation[:-1]:
            text = ''
            tmpp = {}
            if conv["role"] == "user":
                text += "Human: " 
                ...
                text += "Assistant: " 
                tmpp["role"] = "assistant"
            # if len(conv["content"].split()) > 500:
            #     break_flag = 1 
            text += conv['content']
            tmpp["content"] = conv["content"]
            dialog.append(text)
...
def load_nectar(given_prompt, condition, what_type="usual", condition_type="ge",
    start_ind=-1, end_ind=-1):
    # load dataset
    print("hi")
    dataset = load_dataset("allenai/tulu-2.5-preference-data", split="nectar")
    # to pandas
    d = dataset.to_pandas()
    ...
    for i in range(start_ind, end_ind):
        ...
        #    continue
        if chosen_conversation[-1]['content'] == rejected_conversation[-1]['content']:
            print(i)
            print("Chosen:", chosen_conversation[-1]['content'])
            print("Rejected:", rejected_conversation[-1]['content'])
            print("chosen and rejected are the same!")
            continue            
        dialog_cr1.append('Assistant-1: ' + chosen_conversation[-1]['content'])
        dialog_cr1.append('Assistant-2: ' + rejected_conversation[-1]['content'])
        dialog_cr2.append('Assistant-1: ' + rejected_conversation[-1]['content'])
        dialog_cr2.append('Assistant-2: ' + chosen_conversation[-1]['content'])        
        dialog_running = "\n".join(dialog_cr1)
        ...
        # SOTA RM
        tmp_chosen = {"role": "assistant", "content": chosen_conversation[-1]['content']}
        tmp_rejected = {"role": "assistant", "content": rejected_conversation[-1]['content']}
        ...
        model_input_chosen = ip1.format(conversation=dialog_running,
            completion=chosen_conversation[-1]['content'])
        model_input_rejected = ip2.format(conversation=dialog_running,
            completion=rejected_conversation[-1]['content'])
